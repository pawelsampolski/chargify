...
RSpec.describe 'Customers API', type: :request do
  ...
  describe 'POST /customers' do
    # valid payload
    let(:valid_attributes) { { name: 'Paweł Sampolski', address: 'Stolarzowicka ', zip_code: '41-890' } }

    context 'when the request is valid' do
      before { post '/customers', params: valid_attributes }

      it 'creates a customer' do
        expect(json['name']).to eq('Paweł Sampolski')
      end
    ...
    context 'when the request is invalid' do
      before { post '/customers', params: { name: 'Stefan Kalafior' } }
      ...
      it 'returns a validation failure message' do
        expect(response.body)
          .to match(/Validation failed: Address can't be blank, Zip code can't be blank/)
      end
    end
  end
  ...
  describe 'PUT /customers/:id' do
    let(:valid_attributes) { { name: 'Bartuś Boczniak' } }
